...
public class ModelPromptService(ILogger<ModelPromptService> logger)
{
    ...
    public async Task<PromptResponse> GetPromptContent(string promptName, Dictionary<string, object> arguments)
    {
        ...
        {
            case "analyze_model":
                return await GenerateAnalyzeModelPrompt(arguments);
            
            case "review_domain":
                return await GenerateReviewDomainPrompt(arguments);
            
            case "create_model_template":
                return await GenerateCreateModelTemplatePrompt(arguments);
            
            case "validate_structure":
                return await GenerateValidateStructurePrompt(arguments);
            
            case "migration_guide":
                return await GenerateMigrationGuidePrompt(arguments);
            
            default:
                throw new ArgumentException($"Unknown prompt: {promptName}");
        }
    }
