...
public class MockLlmService : ILlmService
{
    ...
    public MockLlmService(ILogger<MockLlmService> logger, IConfiguration configuration)
    {
        ...
        _simulateLatency = _configuration.GetValue<bool>("LLM:Mock:SimulateLatency", true);
        _minLatencyMs = _configuration.GetValue<int>("LLM:Mock:MinLatencyMs", 1000);
        _maxLatencyMs = _configuration.GetValue<int>("LLM:Mock:MaxLatencyMs", 3000);
    }
    ...
    public async Task<LlmUsageEstimate> EstimateUsageAsync(string prompt, string modelId, CancellationToken cancellationToken = default)
    {
        ...
        return new LlmUsageEstimate
        {
            ...
            EstimatedCost = (decimal)totalTokens * 0.002m / 1000m,
            CostCurrency = "USD",
