...
public static class SecurityServiceExtensions
{
    ...
    public static IServiceCollection AddSecurityServices(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        // VSA prompt services
        services.AddSingleton<IVsaPromptService, VsaPromptService>();

        // Audit logging - configure based on settings
        var auditConfig = configuration.GetSection("Security:Audit");
        var auditProvider = auditConfig.GetValue("Provider", "file");
    ...
    IConfiguration configuration) : Microsoft.Extensions.Hosting.BackgroundService
{
    ...
    {
        var auditPath = configuration.GetValue("Security:AuditLogPath", "audit_logs");
        var retentionDays = configuration.GetValue("Security:AuditRetentionDays", 90);
